// components/VapiInterviewBot.tsx
"use client";
...
interface VapiInterviewBotProps {
  interviewPrompt?: string; // Make it optional, so a default can be used if not provided
  interviewType?: string;   // Optional, for displaying the type
}

const publicKey = process.env.NEXT_PUBLIC_VAPI_PUBLIC_KEY || "";

// Accept props here
export default function VapiInterviewBot({ interviewPrompt, interviewType }: VapiInterviewBotProps) {
  ...
  const [assistantId, setAssistantId] = useState<string>(""); // Still allow override via Assistant ID input
  ...
  // Dynamically create the assistant configuration based on the prop
  const dynamicAssistantConfig: CreateAssistantDTO = {
    model: {
      provider: "openai",
      model: "gpt-4o",
      temperature: 0.7,
      messages: [
        {
          role: "system",
          // Use the prompt from props, or a default if not provided
          content: interviewPrompt || "You are a professional and polite AI interviewer named Nexus AI. Your goal is to conduct a structured job interview. Start by welcoming the candidate and asking them to introduce themselves. Then, ask relevant behavioral and technical questions, one at a time. Maintain a neutral, encouraging and respectful tone. Do not interrupt the candidate. If the conversation goes off-topic or the user uses inappropriate language, gently steer them back by saying: 'Let's bring our focus back to the interview, shall we?' Ensure a smooth and professional interview experience. Conclude the interview politely when appropriate, for example, after asking 3-5 questions or if the user indicates they are done.",
        },
      ],
    },
    voice: {
    ...
    // Use the interviewType prop for the name
    name: interviewType ? `Nexus AI ${interviewType.replace('-', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')} Interviewer` : "Nexus AI Interviewer",
  ...
  const handleStartCall = async () => {
    ...
    try {
      ...
      if (assistantId) {
        ...
        callConfig = dynamicAssistantConfig; // Use the dynamically created config
        console.log("[Start Call] Attempting to start call with dynamic configuration.");
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFC] flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8">
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-sm sm:max-w-md lg:max-w-lg relative">
        <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-center text-gray-800">
          AI Interview Bot
        </h1>

        {/* Display the interview type if available from props */}
        {interviewType && (
          <h2 className="text-xl font-semibold mb-4 text-center text-indigo-600">
            Interview Type: {interviewType.replace('-', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
          </h2>
        )}
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-gray-500 mt-1">
            If empty, the component uses the dynamically set interview type or a default `Nexus AI Interviewer` config.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 mb-6">
          {!isCallActive ? (
            <button
              ...
              disabled={isLoading || !publicKey || (!assistantId && !interviewPrompt && !dynamicAssistantConfig.model?.messages?.[0]?.content)}
              className={`flex-1 font-bold py-3 px-4 rounded-lg focus:outline-none focus:shadow-outline transition duration-200
                                ${
                                  isLoading || !publicKey || (!assistantId && !interviewPrompt && !dynamicAssistantConfig.model?.messages?.[0]?.content)
