...
// TensorFlow and BlazeFace imports
...
export default function VapiInterviewBotClient() {
  ...
  const [uploading, setUploading] = useState<boolean>(false); // Used internally, not exposed to UI
  ...
  // --- Video Recording & Face Detection Logic ---
  ...
  useEffect(() => {
    async function loadModel() {
      try {
        ...
        setErrorMessage("Failed to load AI features. Please refresh.");
      }
    }
    loadModel();
  }, []);
  ...
  const startRecording = (stream: MediaStream) => {
    ...
    try {
      mediaRecorderRef.current = new MediaRecorder(stream, { mimeType: 'video/webm; codecs=vp9' }); // Using vp9 for better quality/compression
    } catch (error) {
  ...
  const uploadVideo = async (blob: Blob) => {
    if (!userEmail) {
      ...
      // Optionally, show a subtle message or log, but not a blocking error
      return;
    }
    setUploading(true);
    const formData = new FormData();
    formData.append("video", blob, `interview_${Date.now()}.webm`); // Dynamic filename
    formData.append("email", userEmail);

    console.log("Uploading video in background...");
    // console.log([...formData.entries()]); // Avoid logging large blob data in prod
    ...
    try {
      ...
      if (res.ok) {
        ...
        // alert("Interview uploaded successfully!"); // Removed alert as per requirement
        router.push("/dashboard"); // Redirect after successful upload
      } else {
        let errorText;
        try {
        ...
        // alert("Upload failed: " + errorText); // Removed alert as per requirement
      }
    } catch (error) {
      console.error("Network error during background upload:", error);
      // alert("Error uploading video."); // Removed alert as per requirement
    } finally {
      setUploading(false);
      // No UI update related to 'uploading' state as per requirement
    }
  };
  ...
  const handleStartCall = async () => {
    ...
    try {
      ...
      if (call && call.id) {
        ...
        currentCallIdRef.current = null;
        stopRecording(); // Ensure recording stops if Vapi call fails to start
      }
    } catch (error: any) {
  ...
  const handleStopCall = () => {
    ...
    // handleCallEnd will be triggered by Vapi's 'call-end' event and will call stopRecording()
  };
    ...
    <div className="flex flex-col md:flex-row items-center justify-center min-h-screen p-6 space-y-6 md:space-y-0 md:space-x-4" style={{ backgroundColor: '#FAFAFC' }}>
      ...
      <div className="flex flex-col items-center justify-between p-6 md:p-8 min-h-[450px] w-full max-w-md rounded-2xl">
        <div className="w-full max-w-4xl aspect-[16/12] bg-gray-100 rounded-lg overflow-hidden relative border border-gray-300 shadow-sm flex items-center justify-center">
          ...
          <canvas ref={canvasRef} className="absolute inset-0 w-full h-full"></canvas> {/* Canvas for face detection */}
          {!isCallActive && (
        ...
        <div className="flex space-x-4 mt-auto w-full justify-center">
          {!isCallActive ? (
            <button
              ...
              disabled={isLoading || isScanningResume || !publicKey || !currentJobId || hasInterviewBeenTaken || !model} // Disable if model not loaded
              className={`flex items-center justify-center w-16 h-16 rounded-full text-white text-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-lg
                ${isLoading || isScanningResume || !publicKey || !currentJobId || hasInterviewBeenTaken || !model
